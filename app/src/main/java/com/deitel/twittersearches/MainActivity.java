...
public class MainActivity extends ListActivity
{
   ...
   private EditText urlEditText; // EditText where user enters a favorite website
  // private EditText tagEditText; // EditText where user tags a query
   private SharedPreferences savedSearches; // user's favorite searches
   private ArrayList<String> urls; // list of urls for saved searches
   private ArrayAdapter<String> adapter; // binds urls to ListView
  ...
  //    tagEditText = (EditText) findViewById(R.id.tagEditText);
      ...
      // store the saved urls in an ArrayList then sort them
      urls = new ArrayList<String>(savedSearches.getAll().keySet());
      Collections.sort(urls, String.CASE_INSENSITIVE_ORDER);

      // create ArrayAdapter and use it to bind urls to the ListView
   ...
   public OnClickListener saveButtonListener = new OnClickListener()
   {
      ...
      {
         // create tag if neither urlEditText nor tagEditText is empty
         if (urlEditText.getText().length() > 0) // && tagEditText.getText().length() > 0)
         {
//            addTaggedSearch(urlEditText.getText().toString(),
//               tagEditText.getText().toString());
             addTaggedSearch(urlEditText.getText().toString(),urlEditText.getText().toString());
   ...
   private void addTaggedSearch(String query, String tag)
   {
      ...
      // if tag is new, add to and sort urls, then display updated list
      if (!urls.contains(tag))
      {
         urls.add(tag); // add new tag
         Collections.sort(urls, String.CASE_INSENSITIVE_ORDER);
         adapter.notifyDataSetChanged(); // rebind urls to ListView
      }
   }
   ...
   private void deleteSearch(final String tag)
   {
      ...
      confirmBuilder.setPositiveButton(getString(R.string.delete),
         new DialogInterface.OnClickListener()
         {
            ...
            public void onClick(DialogInterface dialog, int id)
            {
               urls.remove(tag); // remove tag from urls
               ...
               // rebind urls ArrayList to ListView to show updated list
